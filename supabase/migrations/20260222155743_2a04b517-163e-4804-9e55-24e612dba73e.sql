...
-- Drop all existing businesses policies and recreate as PERMISSIVE
...
CREATE POLICY "Users can insert own membership" ON public.business_users
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id OR public.get_user_business_role(auth.uid(), business_id) = 'owner');
