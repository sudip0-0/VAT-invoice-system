...
import {
  ...
  Building2, Search, Menu, LogOut, ArrowLeftRight, Wallet, FilePlus2, Bell, ChevronDown, Banknote, Copy
} from 'lucide-react';
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const { data: alertData } = useReceivablesAlerts();
  useAppShortcuts();
  ...
  const overdueCount = alertData?.count || 0;
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-card px-4">
          ...
          <div className="relative flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {businesses.length > 1 ? (
              <>
                ...
                >
                  <span className="max-w-[10rem] truncate">{business?.name || 'No Business'}</span>
                  <ChevronDown className="h-3.5 w-3.5" />
                </button>
                {businessOpen && (
                  <div className="absolute left-0 top-7 z-50 w-56 overflow-hidden rounded-md border border-border bg-popover shadow-md">
                    {businesses.map((biz) => (
                      <button
                        key={biz.id}
                        type="button"
                        className={`flex w-full px-3 py-2 text-left text-xs hover:bg-muted ${biz.id === business?.id ? 'bg-muted font-medium text-foreground' : 'text-foreground'}`}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button
                type="button"
                aria-label={overdueCount > 0 ? `${overdueCount} overdue invoices` : 'No overdue invoices'}
              ...
              {alertsOpen && (
                <div className="absolute right-0 top-9 z-50 w-80 overflow-hidden rounded-lg border border-border bg-popover shadow-lg">
                  <div className="border-b border-border px-3 py-2 text-xs font-semibold text-foreground">Overdue receivables</div>
                  {overdueCount === 0 ? (
                    <div className="p-3 text-xs text-muted-foreground">No overdue sales invoices.</div>
                  ) : (
                      ...
                      {(alertData?.items || []).map((item) => (
                        ...
                        >
                          ...
                          <span className="text-muted-foreground">
                            {item.buyer_name || 'Customer'} · due {item.due_date_ad} · {formatNPR(item.balance_due, { showSymbol: false })}
                          </span>
                        </button>
                      ))}
                    </div>
