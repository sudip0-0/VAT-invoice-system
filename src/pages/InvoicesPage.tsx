    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="flex items-center gap-1.5 rounded-md border border-border bg-card px-2.5 py-1.5 text-xs text-muted-foreground max-w-xs">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search sales..." className="w-full bg-transparent outline-none text-foreground placeholder:text-muted-foreground" />
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-sm text-muted-foreground">
            {total === 0 ? 'No sales found.' : 'No sales on this page.'}
          </div>
        ) : (
              ...
              <tbody>
                {sales.map((inv) => (
                  <tr key={inv.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => navigate(`/invoices/${inv.id}`)}>
                    ...
                    <td className="px-4 py-3 text-foreground">{inv.buyer_name || inv.customer?.name || '—'}</td>
