...
export default function InvoiceEditPage() {
  ...
  useEffect(() => {
    if (invoice && !initialized) {
      ...
      const parsedDueBs = invoice.due_date_bs ? parseBSShort(invoice.due_date_bs) : null;
      if (parsedDueBs) setDueDateBs(parsedDueBs);
      setDueDateAd(invoice.due_date_ad ? new Date(invoice.due_date_ad).toISOString().slice(0, 10) : '');
      setIsVat(invoice.is_vat_invoice);
      setNotes(invoice.notes || '');

      const existingLines = (invoice.invoice_items || [])
  ...
  return (
    <div className="space-y-4 animate-fade-in max-w-4xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <BSDatePicker
              ...
              className="w-full"
            />
            {issuedDateAd && <span className="text-[10px] text-muted-foreground">AD: {issuedDateAd}</span>}
          </div>
          <div>
            ...
            <tbody>
              {lines.map((line, idx) => (
                <tr key={line.key} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2">
                    <Input type="number" min="0" step="1" value={line.quantity} onChange={(e) => updateLine(line.key, { quantity: Number(e.target.value) })} className="h-8 text-xs text-right w-16 border-0 bg-transparent shadow-none px-0" />
                  </td>
                  <td className="px-3 py-2 text-muted-foreground">{line.unit}</td>
                  <td className="px-3 py-2">
                    <Input type="number" min="0" step="0.01" value={line.rate} onChange={(e) => updateLine(line.key, { rate: Number(e.target.value) })} className="h-8 text-xs text-right w-24 border-0 bg-transparent shadow-none px-0" />
                  </td>
                  <td className="px-3 py-2">
      ...
      <div className="flex gap-2 justify-end pb-4">
        <Button variant="outline" size="sm" onClick={() => navigate(`/invoices/${id}`)}>Cancel</Button>
        <Button variant="secondary" size="sm" onClick={() => handleSave('draft')} disabled={updateInvoice.isPending}>
          Save Draft
        </Button>
